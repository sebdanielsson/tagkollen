...
enum SharedStorage {
    ...
    static let appGroup: String = Bundle.main.object(forInfoDictionaryKey: "TagradarAppGroup") as? String
        ?? "group.se.tagradar.app"
    /// `<TeamID>.<bundle id>`, shared with the widget extension.
    static let keychainAccessGroup: String = Bundle.main.object(forInfoDictionaryKey: "TagradarKeychainGroup") as? String
        ?? "se.tagradar.app"
    private static let logger = Logger(subsystem: "se.tagradar.app", category: "SharedStorage")
    ...
    static var storeURL: URL {
        containerURL.appending(path: "Tagradar.store")
    }
    ...
    static func makeModelContainer() throws -> ModelContainer {
        ...
        migrateLegacyFiles()
        let config = ModelConfiguration("Tagradar", url: storeURL)
    ...
    private static func migrateLegacyFiles() {
        ...
        for name in ["Tagradar.store", "Tagradar.store-shm", "Tagradar.store-wal", "stations-v1.json"] {
